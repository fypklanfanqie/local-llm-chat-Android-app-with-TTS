...
@Entity(tableName = "companion_wallet")
data class CompanionWalletEntity(
...
interface AffinityDao {
    ...
    @Query("SELECT * FROM companion_wallet WHERE id = ${CompanionWalletEntity.SINGLETON_ID}")
    suspend fun getWallet(): CompanionWalletEntity?

    @Query("SELECT * FROM companion_wallet WHERE id = ${CompanionWalletEntity.SINGLETON_ID}")
    fun observeWallet(): Flow<CompanionWalletEntity?>

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsertWallet(entity: CompanionWalletEntity)
    ...
    // ===== 自定义角色特殊邂逅脚本（special_event_script）=====

    @Query("SELECT * FROM special_event_script WHERE characterId = :characterId")
    fun observeScriptsForCharacter(characterId: String): Flow<List<SpecialEventScriptEntity>>

    @Query("SELECT * FROM special_event_script WHERE characterId = :characterId AND threshold = :threshold")
    suspend fun getScript(characterId: String, threshold: Int): SpecialEventScriptEntity?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsertScript(entity: SpecialEventScriptEntity)

    @Query("DELETE FROM special_event_script WHERE characterId = :characterId AND threshold = :threshold")
    suspend fun deleteScript(characterId: String, threshold: Int)

    @Query("DELETE FROM special_event_script WHERE characterId = :characterId")
    suspend fun deleteScriptsForCharacter(characterId: String)
    ...
    suspend fun claimCheckinIfAvailable(dayKey: String, now: Long, reward: Long): Boolean {
        ...
        val wallet = getWallet() ?: CompanionWalletEntity(balance = 0L, updatedAt = now)
...
internal fun CompanionWalletEntity.toDomain() = CompanionWallet(balance, updatedAt)
