...
class SettingsStore(private val context: Context) {
    ...
    val musicPlaylist: Flow<List<BgmTrack>> = context.settingsDataStore.data.map { p ->
    ...
    suspend fun setMusicPlaylist(list: List<BgmTrack>) {
        context.settingsDataStore.edit { it[Keys.MUSIC_PLAYLIST] = voiceJson.encodeToString(list) }
    }

    /**
     * 原子更新播放列表（读-改-写在单个 DataStore edit 事务内，避免并发 lost update）。
     * 上限由 [com.chatbyyourside.data.repository.MusicLibraryRepository] 在导入处强制。
     */
    ...
    /** 随机播放开关。 */
    val musicShuffle: Flow<Boolean> = context.settingsDataStore.data.map { p ->
        p[Keys.MUSIC_SHUFFLE] ?: false
    }

    suspend fun setMusicShuffle(enabled: Boolean) {
        context.settingsDataStore.edit { it[Keys.MUSIC_SHUFFLE] = enabled }
    }
     ...
     * 20 张上限由 [com.chatbyyourside.data.repository.ChatBackgroundRepository] 在 addUris 处强制。
     */
