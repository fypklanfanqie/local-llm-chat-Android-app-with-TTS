...
/**
 * 聊天记录导出图片渲染器（Canvas 直绘，无 View 依赖）。
 *
 * 泛化配色：以大众版 Iris 紫强调色 + 中性深色「聊天壁纸」方案替换方舟金黑主题。
 */
object ConversationImageRenderer {
    ...
    private const val TEXT = 0xFFF1EEE7.toInt()          // 正文米白
    private const val MUTED = 0xFFA9B3C2.toInt()         // 弱化文本
    ...
    private fun drawHeader(canvas: Canvas, document: ConversationExportDocument, pageNumber: Int, pageCount: Int) {
        val title = TextPaint(Paint.ANTI_ALIAS_FLAG).apply { color = ACCENT; textSize = 42f; isFakeBoldText = true }
        val subtitle = TextPaint(Paint.ANTI_ALIAS_FLAG).apply { color = MUTED; textSize = 24f }
        canvas.drawText("聊天记录", INSET.toFloat(), 62f, title)
        ...
        canvas.drawRect(INSET.toFloat(), 156f, (EXPORT_IMAGE_WIDTH_PX - INSET).toFloat(), 160f, Paint().apply { color = ACCENT })
    }
    ...
    private fun drawMessage(canvas: Canvas, message: ConversationExportMessage, top: Int): Int {
        ...
        val isUser = message.senderName == "我" || message.senderName == "用户"
        ...
        val meta = TextPaint(Paint.ANTI_ALIAS_FLAG).apply { color = ACCENT; textSize = 22f; isFakeBoldText = true }
