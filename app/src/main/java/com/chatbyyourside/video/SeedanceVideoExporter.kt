 ...
 * - [exportToMediaStore]：Android 10+ 公共相册 `Movies/ChatByYourSide`；
...
class SeedanceVideoExporter(
     ...
     *
     * `RELATIVE_PATH=Movies/ChatByYourSide` + `IS_PENDING=1→0` 原子发布：
    ...
    suspend fun exportToMediaStore(video: SeedanceVideo): Result<Uri> = withContext(Dispatchers.IO) {
        ...
        val values = ContentValues().apply {
            ...
            put(MediaStore.Video.Media.RELATIVE_PATH, "Movies/ChatByYourSide")
...
/**
 * 建议导出文件名：`ChatByYourSide_{taskUuid前8位}_{任务id}.{ext}`。
...
internal fun suggestedVideoFileName(video: SeedanceVideo): String {
    ...
    return "ChatByYourSide_${video.taskUuid.take(8)}_${video.id}.$ext"
}
