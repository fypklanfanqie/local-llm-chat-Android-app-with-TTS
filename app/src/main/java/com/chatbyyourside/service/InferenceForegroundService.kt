package com.chatbyyourside.service
 ...
 * 本地推理保活前台服务。
 *
 * 由 [com.chatbyyourside.llm.InferenceSessionController] 在 `BackendManager.generate` 期间启停。
 ...
 * - 通知 channel [CHANNEL_ID] `IMPORTANCE_LOW`（无声），在 [ChatApp.onCreate] 经 [createChannel] 创建。
...
class InferenceForegroundService : Service() {
    ...
    companion object {
        ...
        private const val WAKE_LOCK_TAG = "chatbyyourside:inference"
        private const val WAKE_LOCK_TIMEOUT_MS = 10 * 60 * 1000L
        private const val TAG = "InferenceFGService"

        /** 在 [com.chatbyyourside.ChatApp.onCreate] 调用，创建低优先级无声通知渠道。 */
