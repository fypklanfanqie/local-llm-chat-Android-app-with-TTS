...
/**
 * 世界书详情页（独立路由 lorebook/{bookId}）：书名编辑、条目列表（启用开关）、
 * 添加条目、导出本书（SillyTavern 兼容 JSON）。
 */
@Composable
fun LorebookDetailScreen(
    ...
    exportError?.let { message ->
        androidx.compose.material3.AlertDialog(
            ...
            confirmButton = {
                androidx.compose.material3.TextButton(onClick = { exportError = null }) { Text("知道了") }
            },
        )
    }
}
...
private fun ScopePickerDialog(
    book: Lorebook,
    characters: List<com.chatbyyourside.data.model.Character>,
    groups: List<com.chatbyyourside.data.model.Conversation>,
    ...
    androidx.compose.material3.AlertDialog(
        ...
        text = {
            Column {
                com.chatbyyourside.ui.glass.GlassSegmented(
                    options = listOf(
        ...
        dismissButton = {
            androidx.compose.material3.TextButton(onClick = onDismiss) { Text("取消", color = scheme.onSurfaceVariant) }
        },
    )
}
...
private fun RenameDialog(
    ...
    androidx.compose.material3.AlertDialog(
        ...
        text = {
            com.chatbyyourside.ui.glass.GlassTextField(
                value = name,
