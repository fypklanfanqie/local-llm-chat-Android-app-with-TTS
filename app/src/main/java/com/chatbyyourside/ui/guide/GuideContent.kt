package com.chatbyyourside.ui.guide
...
val GUIDE_CATEGORIES: List<GuideCategory> = listOf(
    ...
    GuideCategory("engine_tuning", "⚙️", "引擎与性能调优", "后端 · 参数 · 预热 · 安全模式"),
    GuideCategory("chat_features", "💬", "聊天功能", "会话 · 深度思考 · 图片文件 · 导出"),
    GuideCategory("tts", "🔊", "语音合成 TTS", "朗读 · 音色复刻 · 中日双语"),
    GuideCategory("lore", "📖", "世界观与世界书", "设定注入 · 关键词触发 · SillyTavern"),
    GuideCategory("social", "👥", "角色 · 问候 · 群聊", "角色卡 · 好感度 · 主动消息 · 群聊"),
    GuideCategory("media", "🎵", "音乐 · 视频 · 外观", "网易云 · Seedance · 主题背景"),
)
...
val GUIDE_TOPICS: List<GuideTopic> = listOf(
    ...
    GuideTopic(
        ...
        beginner = listOf(
            para("Chat by your side 是一款 AI 角色扮演聊天应用。内置 20 位性格各异的原创角色，可以像微信一样和她们聊天、送礼物、拉群聊，还能让角色开口说话、给角色拍短视频。"),
            tip("五个页面", "屏幕底部有 5 个入口：通讯（聊天）、角色（管理角色）、朋友圈、模型（本地 AI）、设置；音乐在「设置 → 音乐」里。所有功能都从这里出发。"),
            tip("从哪开始", "打开「通讯」就是一屏角色卡片流，像刷短视频一样上下滑动，停到喜欢的角色上点「开始对话」就能聊。"),
        ),
        experienced = listOf(
            para("基于 LLM 的角色扮演应用：云端走 OpenAI 兼容 /chat/completions（SSE 流式直连），本地走 MNN 端侧推理；20 位原创人设 + 自定义角色卡（JSON 导入导出）。"),
            tip("架构速览", "数据层 DataStore（偏好）+ Room（会话/视频/好感度）；UI 为 Compose 单 Activity + 嵌套导航；后台 WorkManager 驱动主动问候与群聊调度。"),
        ),
    ),
    GuideTopic(
    ...
    GuideTopic(
        ...
        beginner = listOf(
            ...
            stepText("底部点「通讯」，上下滑动卡片流，停在喜欢的角色上。"),
            stepTitle("第 3 步：开始对话"),
            stepText("点「开始对话」进入聊天。欢迎页还有 3 个推荐话题，点一下就能直接发送。"),
        ),
        experienced = listOf(
            para("配置任一 provider（「免费对话」零配置）→ 通讯 Tab 卡片流落定目标角色 → 开始对话；欢迎态提供推荐话题药丸直发。"),
            tip("进阶", "想要更强模型与稳定体验，参照「云端 AI 配置」分类接入 DeepSeek 等；想要完全离线，参照「本地大模型」分类。"),
        ),
    ),
    GuideTopic(
        ...
        beginner = listOf(
            ...
            stepTitle("入口"),
            stepText("角色页右上角「签到」进入「每日供应与商店」；好感度档案页也能进。"),
            tip("送礼物", "聊天输入框旁的 🎁 按钮可以把库存里的礼物当面送给角色，她会当场道谢，好感度上涨。"),
            tip("自制礼物", "商店里点「新建礼物」：起名、写描述、定价（5000–20000 金币），做一份独一无二的礼物。"),
        ),
        experienced = listOf(
            para("好感度经济系统：签到入账 10,000 金币/日（冷启动 DailyCheckinBus 弹窗一次）；礼物购买扣减钱包、赠送走 AffinityRepository 账本并触发 AI 道谢回复（thankYouText 落库）。"),
    ...
    GuideTopic(
        ...
        experienced = listOf(
            para("platform.deepseek.com 创建 Key → 预设 baseUrl https://api.deepseek.com；deepseek-v4-flash 支持思考/非思考双模式与 1M 上下文。"),
            tip("配置记忆", "每个供应商的 baseUrl/model/key 独立记忆（api_config_map），来回切换不串 Key。"),
        ),
    ),
    GuideTopic(
        ...
        beginner = listOf(
            para("除了 DeepSeek，还内置三家服务商的快捷接入。流程都一样：官网注册 → 创建 Key → 回应用里选对应模型商粘贴保存。"),
            stepTitle("OpenAI"),
            stepText("platform.openai.com 创建 Key 并充值 → 模型商选「OpenAI」。需要国外网络环境。"),
            stepTitle("通义千问（阿里百炼）"),
            stepText("bailian.console.aliyun.com 开通服务获取 Key → 模型商选「通义千问」，选 Qwen3.7-Max / Plus 等。"),
            stepTitle("智谱 GLM"),
            stepText("open.bigmodel.cn 获取 Key → 模型商选「智谱 GLM」；GLM-Flash 型号免费。"),
        ),
        experienced = listOf(
            para("三家均为 OpenAI 兼容协议预设（PRESET_PROVIDERS）；baseUrl 与模型清单预置，切换供应商时各自配置独立恢复，互不污染。"),
        ),
    ),
    GuideTopic(
        ...
        experienced = listOf(
            para("任意 OpenAI Chat Completions 兼容端点均可。协议自动识别：URL 含 anthropic/claude 或路径以 /v1/messages 结尾 → /v1/messages + x-api-key；否则 /chat/completions + Bearer。Base URL 经 normalizeBaseUrl 消毒（尾缀 scheme 残渣/空白）；也接受完整 endpoint 直填。"),
        ),
    ),
    GuideTopic(
        ...
        experienced = listOf(
            para("SiliconFlow 免费共享 7B（Qwen/Qwen2.5-7B-Instruct 与 DeepSeek-R1-Distill-Qwen-7B），经 Cloudflare Worker 代理转发；真实 Key 存于 Worker Secret 由服务端注入，App 端 requiresApiKey=false 允许空 Key 落库与请求。"),
        ),
    ),
    GuideTopic(
        ...
        experienced = listOf(
            para("保存走 saveApiConfig 原子双写：更新活跃 api_config 三键 + 写入该供应商记忆槽；密码框 VisualTransformation 遮蔽显示。"),
        ),
    ),
    ...
    GuideTopic(
        ...
        experienced = listOf(
            para("MNN 格式共 14 个：Qwen3.5 系列 0.8B~35B-A3B（MoE）、Qwen3-4B、DeepSeek-R1 蒸馏系（1.5B / 7B / 8B-0528）、Llama-3.2-1B、Gemma 系等；「模型」页刷新按钮可重新拉取列表。"),
            tip("版本标识", "模型卡显示名称 / 推荐星标 / 体积 / MNN / 版本号；下载完成后才可「切换使用」。"),
        ),
    ),
    GuideTopic(
        ...
        beginner = listOf(
            ...
            tip("失败怎么办", "网络波动导致失败时，卡片会出现「重试」按钮，点击继续即可，已下载的部分不会浪费。"),
            tip("下载太慢", "模型托管在公共源，高峰期偏慢属正常；换Wi-Fi 或错峰再试。"),
        ),
        experienced = listOf(
            para("分块下载 + 断点续传；完成后进入「校验中」再做完整性校验；各失败阶段给出对应重试入口。模型文件不占用「存储管理」统计口径（后者仅业务数据）。"),
        ),
    ),
    GuideTopic(
        ...
        experienced = listOf(
            para("切换写入 active_local_model_id；switchProvider(LOCAL) 时按推理引擎设置快照加载；生成期间 InferenceForegroundService 低优先级保活通知。"),
        ),
    ),
    GuideTopic(
        ...
        experienced = listOf(
            para("KV cache 按 fp16（2 字节/元素）估算：bytes = 2 × layerCount × contextLen × kvHeads × headDim（GQA 按 numKeyValueHeads 折减，未知结构回退 full-hidden 近似）；MoE 模型（Qwen3.5-35B-A3B）不入兜底表、不做估算。实现在 LlmMemoryEstimator。"),
        ),
    ),
    ...
    GuideTopic(
        ...
        experienced = listOf(
            para("AUTO = GPU 优先回退 CPU，且 AUTO GPU 严格限定 >7B 模型准入；OpenCL GPU 走健康探测与冷却机制（BackendHealthCoordinator 维护健康记录，异常后端移出回退链）；QNN NPU 基于 Hexagon（需骁龙 + 解锁）。完整回退链预览见推理引擎设置页底部说明区。"),
        ),
    ),
    GuideTopic(
        ...
        experienced = listOf(
            para("InferenceProfileResolver 统一裁决 BALANCED/MAXIMUM_SPEED 与候选加速项。Lookahead 仅当 device+model+variant+native 组合命中 CPU_OPTIMIZED 认证记录（InferenceCertificationStore）时启用；未认证候选恒回落 lookahead=false / step=1。"),
            tip("基准闭环", "「运行基准并认证」跑 FIXED_DECODE 对比（预热 1 + 记录 3 轮）→ ExperimentalPromotionPolicy 判定 promotion → 认证落盘后旧开关才被 resolver 放行。"),
        ),
    ),
    GuideTopic(
        ...
        beginner = listOf(
            ...
            tip("最大生成长度", "单次回复的字数上限，可选 1024 / 2048 / 4096 / 不限。"),
            warn("修改后下次发消息时自动重载生效，页面顶部会显示提示横幅。"),
        ),
        experienced = listOf(
            para("默认 contextLen=4096 / threads=4 / temperature=0.8 / repeatPenalty=1.2（AppConfig.LLM）。KV cache 随 contextLen 线性增长（见「内存占用怎么看」）；llm_config_changed 横幅提示重载，成功推理后 acknowledge 写回 last_applied 快照消除横幅。"),
        ),
    ),
    GuideTopic(
        ...
        beginner = listOf(
            para("用 GPU 时是不是感觉第一句话特别慢？到 推理引擎设置 → 高级 里手动点一次「GPU 完整预热」，之后首次回复会明显变快。"),
            warn("只有大于 7B 的大模型才需要预热（小模型按钮不可用）；预热过程手机发热几分钟属正常现象。"),
        ),
        experienced = listOf(
            para("GpuPreheatCoordinator 手动触发（空闲期策略仅轻量 OpenCL 探测、绝不自动加载模型）：加载当前 >7B 模型执行极短 GPU 生成，预热 OpenCL 图/内核/缓存，降低首次出字延迟；不影响聊天记录与已保存设置。"),
        ),
    ),
    GuideTopic(
        id = "safe_mode", categoryId = "engine_tuning", emoji = "🛡️",
        title = "崩溃安全模式",
        aliases = listOf("安全模式", "闪退", "崩溃", "打不开", "crash", "崩溃日志", "一直闪退"),
        beginner = listOf(
            para("如果应用连续两次启动都闪退，下一次会自动进入「安全模式」：跳过最容易受驱动问题影响的组件，先保证应用打得开、能用。"),
            tip("怎么恢复", "安全模式下正常使用、不再连续闪退后会自动恢复正常模式。"),
            tip("崩溃日志在哪", "设置 → 崩溃日志：列出每次崩溃的日志，点任意条目即可系统分享给开发者，无需连接电脑。"),
        ),
        experienced = listOf(
            para("CrashWatchdog 以启动窗口内 started→loaded 标记间接判定原生崩溃；连续 ≥ SAFE_MODE_THRESHOLD(2) 次进入降级启动：跳过 :mnn_probe 隔离进程 OpenCL 探测与后台调度。崩溃日志对话框头部附启动诊断摘要（上次阶段 / 连续崩溃计数 / 安全模式状态）。"),
        ),
    ),
    GuideTopic(
        ...
        beginner = listOf(
            para("本地模式下聊天页左上角有一个小浮窗，实时显示 AI 出字速度、手机温度、内存占用等信息。纯展示、不影响聊天，可以拖到顺手的位置。"),
            tip("外观切换", "设置 → 本地 AI 引擎 → 性能浮窗液态玻璃：开=模糊玻璃质感（Android 12+），关=普通深色面板。老手机建议关。"),
        ),
        experienced = listOf(
            para("PerformanceCollector 500ms 周期采集：token 速率、CPU 大核频率、GPU/NPU 状态、温度、内存六类指标；液态玻璃渲染走 RenderEffect 模糊路径，关闭后为纯色面板。"),
        ),
    ),
    ...
    GuideTopic(
        ...
        beginner = listOf(
            para("想把聊天记录保存或分享？点 💬 打开会话抽屉 →「导出记录」→ 选会话、选格式："),
            tip("TXT", "完整文字记录，方便备份与搜索。"),
            tip("图片", "把聊天渲染成漂亮的对话截图：超长会话选「自动分页多张图」，短会话可以拼「一张超长图」（过长会被提示改用分页）。"),
        ),
        experienced = listOf(
            para("conversationexport 管线：TXT 全量落盘 / Canvas 渲染 PNG；SAF OpenDocumentTree 写出，长图超限引导分页防 OOM。"),
        ),
    ),
    GuideTopic(
        ...
        beginner = listOf(
            ...
            tip("什么时候开", "数学、逻辑、复杂问题更适合；日常闲聊关掉回复更快。"),
            warn("不是所有模型都会思考：免费的 DeepSeek-R1-7B、云端的 Qwen / DeepSeek 系列支持较好。"),
        ),
        experienced = listOf(
            para("云端解析 delta.reasoning_content / reasoning 字段包装 <think> 流式段；本地 MNN enable_thinking 且受 LocalThinkingLevel 档位（自动/短/中/长）控制。开关切换即时重渲染历史消息。"),
        ),
    ),
    GuideTopic(
        ...
        beginner = listOf(
            ...
            warn("需要支持看图的模型：云端多模态模型（如 GPT-4o、GLM-4V 等）可以；免费通道与本地模型目前看不了图片。"),
        ),
        experienced = listOf(
            para("图片 base64 进 vision content parts；能否携带由活跃模型的模态能力决定，本地 MNN 无 vision。输入栏九宫格预览可逐张移除。"),
        ),
    ),
    GuideTopic(
        ...
        experienced = listOf(
            para("DocumentRepository 逐页渲染提取（页数上限），base64 流式编码防大文件 OOM；Office 族不受支持需预转 PDF。"),
        ),
    ),
    GuideTopic(
        ...
        experienced = listOf(
            para("CodeBlockView 语法高亮 + 折叠 + 复制；MathView 渲染行内 $...$ 与块级 $$...$$ LaTeX（上下标/分数/根号常用子集）。"),
        ),
    ),
    ...
    GuideTopic(
        ...
        beginner = listOf(
            ...
            tip("先试听", "设置里有「试听」按钮，配好后点一下就知道效果；B 站视频教程在 TTS 区域下方入口。"),
        ),
        experienced = listOf(
    ...
    GuideTopic(
        ...
        experienced = listOf(
            para("VoicePair(zh/ja speaker_id) 按角色映射（tts_voice_map JSON）；解析顺序：角色当前语言 speaker_id → TtsConfig.defaultVoiceId；JA 缺失同样回落默认。试听按当前引擎与语言取样。"),
        ),
    ),
    GuideTopic(
        ...
        experienced = listOf(
            para("JA 模式：正文生成后经 chatOnce 非流式翻译（translateToJapanese）再合成；字幕条同步展示原文与译文。TtsLanguage 全局持久化，问候等被动消息不受影响。"),
        ),
    ),
    GuideTopic(
        ...
        beginner = listOf(
            ...
            tip("停止朗读", "朗读中再点一次 ▶ 即停止；开始播放角色视频时会自动暂停朗读避免抢声。"),
        ),
        experienced = listOf(
            para("tts_auto_read 持久化开关；TtsManager 队列化 utterance，Seedance 播放控制器抢占时挂起当前朗读。"),
        ),
    ),
    ...
    GuideTopic(
        ...
        beginner = listOf(
            para("「世界观」是你写给 AI 看的背景设定，比如「这个世界魔法是违法的」「我和她是同学」。每条设定只对你指定的那个聊天生效。"),
            stepTitle("怎么建"),
            stepText("设置 → 世界观设定 → 添加世界观：起名、写正文（最多 4000 字）、选择给谁用（某角色的私聊或某群）→ 保存。"),
            tip("多条叠加", "同一个聊天可以添加多条世界观，按顺序依次生效；目标聊天被删时会明确提示「目标不存在」。"),
        ),
        experienced = listOf(
    ...
    GuideTopic(
        ...
        beginner = listOf(
            ...
            stepTitle("导入现成的"),
            stepText("设置 → 世界书 → 总开关旁「导入 .json」：SillyTavern（酒馆）格式的世界书文件直接兼容。"),
            stepTitle("自己建"),
    ...
    GuideTopic(
        ...
        beginner = listOf(
            ...
            tip("蓝灯 = 常驻", "条目编辑页的「常驻条目」开关（酒馆里叫蓝灯）：打开后不再需要关键词，每次对话必定注入。核心世界观设定用它。"),
            tip("插入位置", "可放在角色设定的前面 / 后面，或插进最近聊天记录中间的指定深度（@深度，数值越小越靠近最新消息、影响越强）。"),
    ...
    GuideTopic(
        ...
        experienced = listOf(
            para("budgetCapTokens ≤0 视为不限——本地默认 context 4096 下危险，PromptWindowPlanner 有 SYSTEM_PROMPT_TOO_LARGE 兜底报错；recursiveScanning 最多 3 轮；条目级 preventRecursion / excludeRecursion 分别阻止自身外链扩散与被递归轮激活。"),
        ),
    ),
    ...
    GuideTopic(
        ...
        title = "角色库与搜索",
        aliases = listOf("角色", "人设", "角色库", "换角色", "20位", "persona", "角色搜索", "立绘"),
        beginner = listOf(
            para("应用内置 20 位原创角色，每人有独立性格、说话方式与立绘。「通讯」页上下滑动卡片挑人；「角色」页网格浏览，顶部搜索框支持按名称 / 代号 / 职位 / 种族查找。"),
            tip("查看人设", "角色卡上的「查看人设」可阅读完整人格设定正文。"),
            tip("点谁聊谁", "角色页点任意角色卡片，即刻把她设为当前对象并直达聊天。"),
        ),
        experienced = listOf(
            para("Characters.ORDER 固定序列 + 自定义角色合并展示；filterCharacters(name/code/role/race contains，忽略大小写) 同函数复用于角色页与世界观点选器。"),
        ),
    ),
    GuideTopic(
        ...
        beginner = listOf(
            para("想聊自己创造的角色？角色页 →「新建」：起名（代号 / 职位 / 种族可选）、传一张立绘、写一段「人格设定」（她是谁、什么性格、怎么说话），保存后立刻出现在卡片流里。"),
            tip("分享角色", "「导出」把你做的角色复制成 JSON 发给别人；「导入」粘贴 JSON 收下对方的角色。"),
            tip("随时改", "自定义角色卡片角上有编辑 / 删除按钮，随时调整人设。"),
        ),
        experienced = listOf(
            para("Character(systemPrompt 必填) JSON 序列化导入导出（exportCustom / 批量 import）；自定义角色立绘经 CharacterImageStore 本地存储；特殊邂逅各阶段剧情文案亦可编辑（EventEditDialog）。"),
        ),
    ),
    GuideTopic(
        ...
        beginner = listOf(
            ...
            tip("关系档案", "角色页点「好感 X / 200」进入档案页：进度条、收礼记录（礼物墙）、随好感解锁的剧情入口。"),
            tip("特殊邂逅", "达到好感门槛解锁专属剧情对话，类似游戏里的个人线；自定义角色可以自己编写各阶段剧情。"),
            tip("我的形象", "设置 → 我的形象：上传你的头像、写下你是谁、与她什么关系——AI 在单聊、群聊、主动消息里都会记住这些设定。"),
        ),
        experienced = listOf(
            para("好感经济为 Room 体系（钱包 / 账本 / 礼物档案）；AFFINITY_EVENT_THRESHOLDS 阈值解锁阶段事件，SpecialEventCatalog 场景 prompt 开启事件会话（内置角色 JSON 目录 + 离线保底）；UserProfileConfig(persona/relationship/avatar) 注入三类会话上下文。"),
        ),
    ),
    GuideTopic(
        ...
        experienced = listOf(
            para("GreetingScheduler 15min PeriodicWork 心跳 + next_fire_at 门控 + 精确闹钟兜底 + BootReceiver 重挂；投递时段 08:00–23:00；每日配额默认 3（1–10）；多角色按 greeting_last_char_id 严格轮询；Android 14 生成期前台化通知；测试路径跳过门控不计配额。"),
        ),
    ),
    GuideTopic(
        ...
        beginner = listOf(
            para("国产手机为省电喜欢「杀后台」，会导致主动问候 / 群聊收不到。设置 → 角色问候 里备好三个跳转按钮，逐一点击去系统里放行："),
            stepTitle("三步设置"),
    ...
    GuideTopic(
        ...
        beginner = listOf(
            para("可以拉多个角色建一个群一起聊！通讯页顶部「群聊」→ 新建：选封面、起群名、勾选成员。"),
            tip("@提人", "输入 @ 可以点名某个角色回答，她一定回；其他成员也可能随机搭话（一条消息最多 4 名成员回复）。"),
            tip("自动聊天", "设置 → 群聊 → 打开「空闲自动聊天」：你不说话时群里成员也会自己唠起来，偶尔还会向你提问；每天轮次可调（1–20，默认 8）。"),
            warn("群聊仅云端 AI 可用；正盯着群聊页面时通知不会打扰你。"),
        ),
        experienced = listOf(
            para("GroupChatWorker：discuss 轮 DISCUSS_REPLIES_PER_ROUND=2 名成员接力互聊；每 ASK_USER_EVERY_N_ROUNDS(3) 轮改为 ask-user 轮向用户提问。用户回合 MAX_REPLIES_PER_USER_MESSAGE=4（@ 指定必答占额 + 随机补齐）；成员上限 10、人设截 300 字符、上下文 40 条；时段 09:00–22:00、用户发言后 1h 冷却；GroupScreenTracker 前台抑制通知。"),
        ),
    ),
    ...
    GuideTopic(
        ...
        beginner = listOf(
            para("「设置 → 音乐」顶部搜索框可以搜网易云在线听：结果里点「添加」收进播放列表，随时播放。"),
            tip("全局播放", "音乐在后台持续播放，聊天、刷角色都不打断；角色视频出声时会自动让路暂停。"),
        ),
        experienced = listOf(
            para("网易云接口搜索取外链 → ExoPlayer 流式播放；播放列表持久化（MUSIC_PLAYLIST）；与 Seedance 播放互斥降音（音频焦点协调）。"),
        ),
    ),
    GuideTopic(
        ...
        beginner = listOf(
            para("「设置 → 音乐」点「导入本地音乐」可从手机里选歌（支持多选），导入后保存在应用内，完全离线可听。"),
    ...
    GuideTopic(
        ...
        beginner = listOf(
            para("播放器支持：随机 / 上一曲 / 播放暂停 / 下一曲 / 循环模式切换（顺序 · 列表循环 · 单曲循环）、进度条拖动、音量滑杆。"),
            tip("歌词", "有歌词的歌会逐行高亮自动滚动居中，当前句带流光效果；本地导入的歌若无歌词则显示「暂无歌词」。"),
        ),
        experienced = listOf(
            para("LRC 解析 + 当前行居中滚动（LyricView）；repeatMode int 持久化（0 顺序 / 1 列表循环 / 2 单曲循环）。"),
        ),
    ),
    GuideTopic(
        id = "seedance_video", categoryId = "media", emoji = "🎬",
        title = "Seedance 对话视频与邂逅",
        aliases = listOf("视频", "seedance", "生成视频", "自动视频", "邂逅", "对话视频", "短片"),
        beginner = listOf(
            para("想看角色「动起来」？聊天顶栏点 📹 开启自动视频：之后 AI 每次回复都会自动生成一段对应短视频插在消息里（需先配置服务）。"),
            stepTitle("配置"),
            stepText("设置 → Seedance 对话视频：填 API Key（火山方舟官方，或支持媒体协议的中转站）→ 点「测试连接」→ 保存。"),
            tip("看所有视频", "通讯页顶部「邂逅」：生成过的视频按时间排成竖滑流，可全屏播放、保存到手机。"),
            tip("失败了", "视频卡上有中文状态说明，按提示「重试 / 重新提交 / 继续查询 / 重新下载」即可；涉及扣费的重试会先弹确认。"),
            warn("仅云端 AI 模式可用；生成需要排队，等待几分钟属正常。"),
        ),
        experienced = listOf(
            para("双协议自适应：官方方舟 base（含 /api/v3）任务接口；中转站媒体协议 /v1/media/generate + /v1/media/status（URL 特征自动识别）。SeedanceVideoState 状态机分阶段给出对应重试动作，WorkManager 后台推进；Fast 变体仅 480/720p；水印仅官方渠道生效；语音固定开启。"),
        ),
    ),
    GuideTopic(
        ...
        experienced = listOf(
            para("ThemeMode(SYSTEM/LIGHT/DARK) 持久化，MainActivity 冷启动同步读取初始化 SystemBarStyle，避免系统栏图标错位闪烁。"),
        ),
    ),
    GuideTopic(
        ...
        beginner = listOf(
            para("设置 → 聊天背景：打开开关、从相册挑选最多 20 张图片，聊天时背景每 8 秒自动轮换一张，切换带淡入淡出效果。"),
            tip("性能浮窗外观", "设置 → 本地 AI 引擎 → 性能浮窗液态玻璃：开 = 炫酷磨砂玻璃，关 = 朴素深色面板（老手机建议关）。"),
        ),
        experienced = listOf(
            para("ChatBackgroundRepository(MAX_BACKGROUNDS=20)：URI 拷贝至内部存储规避持久化权限问题；ChatBackground 8s Crossfade 轮播；liquid_glass_perf_overlay 控制 RenderEffect 模糊渲染路径（Android 12+）。"),
        ),
    ),
)
 ...
 * 同分按标题稳定排序。话题总量 ~46，内存即时过滤即可，无需 debounce。
 */
