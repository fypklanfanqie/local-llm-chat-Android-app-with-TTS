package com.chatbyyourside.ui.navigation
    ...
    androidx.compose.material3.Scaffold(
        ...
        NavHost(
            ...
            composable(BottomTab.Moments.route) {
                // 朋友圈作为 dock 根页：与通讯 Tab 一样全出血沉浸设计（自带顶栏与底栏留白），
                // 故不套 tabBottomPadding；bottomBarHeight 交由页面内部给列表垫底。
                // onBack = null：根页没有上一级，隐藏顶栏返回键（从卡片流进入时仍显示）。
                MomentsScreen(
                    container = container,
                    bottomBarHeight = bottomBarHeight,
                    onBack = null,
            ...
            composable(MUSIC_ROUTE) {
                ...
                Box(tabBottomPadding) { MusicScreen(container = container, onBack = { navController.popBackStack() }) }
            }
