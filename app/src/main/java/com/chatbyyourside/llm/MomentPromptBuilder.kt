...
object MomentPromptBuilder {
     ...
     * ——caption 是要发的朋友圈文字（第一人称、口语化、纯日常分享，不围绕用户展开）；
    ...
    fun buildPostUserMessage(
        ...
        append("\n2. caption 是朋友圈正文：第一人称、口语化、符合你的人设与心情的日常分享（今天做的事、心情、见闻、吃喝、吐槽等），1~3 句话，不要话题标签，不要出现「朋友圈」三个字或任何元叙述；不要提到用户，不要写任何和用户有关的事（@ 好友写出名字除外）。")
        ...
        if (imageCount > 0) {
            append("\n4. imagePrompt 是给生图模型的英文生图提示词：描述一张适合配这条朋友圈的画面（日系动画插画风格：干净线稿、赛璐璐上色、游戏 CG 质感，不要写实照片），不要出现人物面部特写以外的奇怪元素，不要文字水印。")
        } else {
            append("\n4. imagePrompt 填空字符串。")
        }
    ...
    fun buildPostSystemDirective(mentionTarget: String?): String = buildString {
        append("\n\n[朋友圈基调] 这是你自己的日常分享（今天做的事、心情、见闻、吃喝、吐槽等），与用户无关：不要提到用户，不要写任何和用户有关的事；唯一例外是 @ 好友时写出 TA 的名字。")
        if (!mentionTarget.isNullOrBlank()) {
     ...
     * 风格固定为日系动画插画（干净线稿/赛璐璐上色），不用写实照片。
     */
    fun buildImageGenUserMessage(imagePrompt: String, imageCount: Int): String = buildString {
        append("请以参考图中的人物为同一角色（保持发型、服装风格与气质一致），生成 $imageCount 张日系动画插画风格的插画：")
     ...
     * [userDisplayName] = 用户在社交场景的显示名（「我的形象」昵称，空则「我」）。
     */
