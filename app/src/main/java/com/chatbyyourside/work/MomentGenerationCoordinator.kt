...
import com.chatbyyourside.llm.MomentPromptBuilder
import com.chatbyyourside.util.MomentImageExtractor
...
class MomentGenerationCoordinator(
    ...
    private val chatRepository: com.chatbyyourside.data.repository.ChatRepository,
    private val conversationRepository: com.chatbyyourside.data.repository.ConversationRepository,
    private val characterRepository: com.chatbyyourside.data.repository.CharacterRepository,
    ...
    suspend fun generateAndPost(
        ...
        // 有的时候随机 @ 一个人（其他角色或用户）；其余帖子不 @。内容一律纯日常、不提用户。
    ...
    suspend fun generateReply(
        ...
        val system = buildString {
            ...
            append(com.chatbyyourside.llm.OutputLanguage.ZH_DIRECTIVE)
        }
        val raw = withTimeout(AppConfig.Moment.GENERATE_TIMEOUT_MS) {
            directLlmClient.chatOnce(
                baseUrl = apiConfig.baseUrl,
    ...
    private suspend fun resolveReferenceImage(characterId: String, character: com.chatbyyourside.data.model.Character): String? {
        if (character.isCustom && character.image.isNotBlank()) return character.image
        return AssetPathsHolder.pictureOf(characterId)
    }

    /** 组装发圈 caption 的 system 提示词：人设 + 日常基调（不提用户）+ 本条 @ 规则。 */
    private suspend fun buildCaptionPrompt(
        ...
        if (imageCount > 0) {
            append("imagePrompt 是英文生图提示词，描述你要配图的画面（日系动画插画风格：干净线稿、赛璐璐上色，不要写实照片）。")
        } else {
     ...
     * 从「其他角色 + 用户（昵称，未设置则『我』）」里均匀随机选一个；否则返回 null（本条不 @）。
     */
    private suspend fun rollMentionTarget(posterName: String): String? {
        ...
        val candidates = characterRepository.getAllNamesNow(excludeName = posterName) +
            settings.getUserProfileNow().displayName.trim().ifBlank { "我" }
...
private object AssetPathsHolder {
    fun pictureOf(characterId: String): String? =
        com.chatbyyourside.config.AssetPaths.PICTURES[characterId]?.let { "file:///android_asset/$it" }
}
